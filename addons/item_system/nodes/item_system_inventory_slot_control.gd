...
func _ready() -> void:
	if is_draggable:
		...
		add_child(draggable)
		draggable._get_drag_data_substitute = func(): return item_stack.duplicate()
		draggable._get_drag_preview_substitute = func(): var preview = duplicate(true) ; preview.size = size ; return preview
	...
	if is_droppable:
		...
		add_child(droppable)
		droppable.data_dropped.connect(_receive_data_substitute)
		droppable._receive_data_substitute = _receive_data_substitute
		droppable._can_drop_data_substitute = _can_drop_data_substitute
	_update_state()
...
func _update_state():
	...
	text = item_stack.item.name if item_stack.item else ""
	queue_redraw()
...
func _can_drop_data_substitute(_at_position:Vector2, data:Variant) -> bool:
...
func _receive_data_substitute(_at_position: Vector2, data: Variant) -> Error:
