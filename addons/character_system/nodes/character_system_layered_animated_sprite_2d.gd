@tool
class_name CharacterSystem_LayeredAnimatedSprite2D extends LayeredAnimatedSprite2D
...
func _fine_tune_children() -> void:
	var skin_layers: Array[CharacterSystem_SkinItem] = skin.get_layers_in_drawing_order()
	var layers_children = get_children().filter(func(el): return el is Sprite2D)
	var current_layer = 0
	skin_layers.reverse()
	for layer in skin_layers:
		if layer.frames_bg:
