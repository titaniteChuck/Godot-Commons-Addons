...
func _trigger_craft():
	var matching_recipes: Array[ItemSystem_Recipe] = get_recipe_from_ingredients(ingredients_slots.inventory.item_stacks)
	if not matching_recipes.is_empty():
		ItemSystem.RecipeService.craft_recipe(matching_recipes[0], ingredients_slots.inventory, results_slots.inventory)
